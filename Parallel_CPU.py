...
#Split the poind cloud in small sizes to do parallelisation 
...
def split(A,B):
    ...
    end_array = np.append(xyz,final,axis=1)
    return end_array

# This function will be execute as parallel code

def full(i):
    c=split(alpha1[i],alpha2[i])
    return c

#Create the pool executor and keep the data in a new array

with concurrent.futures.ProcessPoolExecutor() as executor:
    idn = np.arange(len(alpha1))
    results=executor.map(full,idn)

    for result in results:
        x=result
        delta=np.append(delta,x,axis=0)
        
#This variable has all the Descriptors and x,y,z values
