...
def split(A,B):
    ...
    for j1 in range(point_ID_max):
      Shannon_entropy_real = np.zeros([1,num_k])
      for j2 in range(num_k):
          
          # select neighboring points
          
          P = idx[j1,1:int(K[j2])+1]          # the point and its k neighbors ...
          
          #Send all the necesary data to the GPU and do all the maths inside the function calculoEntropia
          
          Shannon_entropy_real[0,j2] = CalculoEntropia(P,x,y,z)
...
#This function do all the calculates  on the GPU to get the Shannon Entropy

def CalculoEntropia(Vector,array_x,array_y,array_z):
    x=array_x
    y=array_y
    z=array_z
    
    Arry = cp.array([x[Vector],y[Vector],z[Vector]])
    Covarianza = cp.cov(Arry)
    eigenValues = cp.linalg.eigvalsh(Covarianza)
    epsilon_to_add = 1e-8;
    if eigenValues[2] <=0:
        eigenValues[2] = epsilon_to_add
    if eigenValues[1] <=0:
        eigenValues[1] = epsilon_to_add
    if eigenValues[0] <=0:
        eigenValues[0] = epsilon_to_add
    EVs = 1.0*eigenValues/cp.sum(eigenValues);
    Shannon_entropy_cal = -( EVs[0]*cp.log(EVs[0]) + EVs[1]*cp.log(EVs[1]) + EVs[2]*cp.log(EVs[2]) )
    Entropia = cp.real(Shannon_entropy_cal)
    Entropia_cpu = cp.asnumpy(Entropia)
